...
extern void __nbody_cuda_mpi_naive(int n, int m, float dt, particle_t parts[], float grav, FILE* fp, bool full_output);
...
void nbody_cuda_mpi_naive(int n, int m, float dt, particle_t parts[], float grav, FILE* fp, bool full_output) {
    __nbody_cuda_mpi_naive(n, m, dt, parts, grav, fp, full_output);
}
